...
test(
  "Vercel-managed pool supports concurrent queries and transaction rollback",
  {
  ...
  async () => {
    ...
    try {
      const results = await Promise.all(
        Array.from({ length: 8 }, () => prisma.$queryRaw`SELECT 1 AS value`),
      );
      for (const result of results) assert.deepEqual(result, [{ value: 1 }]);
      await assert.rejects(
        prisma.$transaction(async (tx) => {
          await tx.organization.create({
            data: { id, slug: `pool-test-${id}`, name: "Pool test" },
          });
