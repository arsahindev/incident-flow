...
test(
  "public demo seed is repeatable, permits incident work, and cannot administer access",
  {
  ...
  async () => {
    ...
    const account = {
      ...publicDemoAccount("dev"),
      id: randomUUID(),
      email: `demo-${randomUUID()}@example.com`,
    };
    ...
    try {
      ...
      assert.equal(
        await prisma.teamMembership.count({ where: { userId: account.id } }),
        1,
      );
      ...
      const headers = {
        authorization: `Session ${login.json().session.token}`,
      };
      ...
      const invitation = await app.inject({
        ...
        url: "/v1/invitations",
        headers,
        payload: { email: "visitor@example.com", role: "admin" },
      });
      assert.equal(invitation.statusCode, 403);
      const promote = await app.inject({
        method: "PATCH",
        url: `/v1/members/${account.id}`,
        headers,
        payload: { role: "owner" },
      });
      ...
      const incident = await app.inject({
        ...
        url: "/v1/incidents",
        headers,
        payload: { title: "Demo incident", teamId, serviceIds: [serviceId] },
      });
      ...
      await assert.rejects(
        seedDemoAccount(prisma, account, organizationId, teamId, pepper),
