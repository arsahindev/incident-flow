...
test(
  "demo incidents have service links and history, and reseeding preserves visitor edits",
  {
  ...
  async () => {
    ...
    try {
      await prisma.organization.create({
        data: {
          id: organizationId,
          name: "Demo content test",
          slug: `demo-content-${organizationId}`,
        },
      });
      ...
      const incidents = await prisma.incident.findMany({
        ...
        include: { affectedServices: true, activity: true },
      });
      ...
      for (const incident of incidents) {
        assert.equal(
          incident.affectedServices.filter((service) => service.isPrimary)
            .length,
          1,
        );
      ...
      await prisma.incident.update({
        where: { id: edited.id },
        data: { title: "Visitor edit", version: { increment: 1 } },
      });
      await seedDemoIncidents(prisma, organizationId, teamId, actorUserId);
      assert.equal(
        await prisma.incident.count({ where: { organizationId } }),
        6,
      );
      const preserved = await prisma.incident.findUniqueOrThrow({
        where: { id: edited.id },
      });
      assert.equal(preserved.title, "Visitor edit");
      assert.equal(preserved.version, edited.version + 1);
      assert.equal(
        await prisma.incidentActivity.count({ where: { organizationId } }),
        10,
      );
    } finally {
