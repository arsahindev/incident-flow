...
export default async function ServicePage({
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      <Link href="/services" className="text-sm text-cyan-300">← Back to services</Link>
      {updated === "1" ? (
        <p
          role="status"
      ...
      <div className="mt-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.18em] text-slate-500">{service.type} · {service.tier} tier</p>
          <h1 className="mt-2 text-3xl font-semibold">{service.name}</h1>
          <p className="mt-3 max-w-2xl text-slate-400">{service.description || "No description provided."}</p>
        </div>
        <div className="text-right text-sm text-slate-400">
          <p>Status: {service.status}</p>
          <p className="mt-1">Owner: {service.ownerTeam?.name ?? "Unassigned"}</p>
        </div>
      </div>
      ...
      <div className="mt-10 grid items-start gap-8 lg:grid-cols-[1fr_22rem]">
        <div className="space-y-8">
          <section className="rounded-xl border border-slate-800 bg-slate-900">
            <div className="border-b border-slate-800 px-6 py-4"><h2 className="font-semibold">Environments</h2></div>
            <ul className="divide-y divide-slate-800">
              {service.environments.map((environment) => {
                const archiveAction = archiveServiceEnvironmentAction.bind(null, service.id, environment.id);
                return (
                  <li key={environment.id} className="flex items-center justify-between gap-4 px-6 py-5">
                    <div>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900">
            <div className="border-b border-slate-800 px-6 py-4"><h2 className="font-semibold">Incident history</h2></div>
            {service.recentIncidents.length === 0 ? <p className="px-6 py-8 text-sm text-slate-400">No incidents have affected this service.</p> : (
              <ul className="divide-y divide-slate-800">
                {service.recentIncidents.map((incident) => (
                  <li key={incident.id}><Link href={`/incidents/${incident.id}`} className="flex items-center justify-between gap-4 px-6 py-5 hover:bg-slate-800/50">
                    <div><p className="font-medium">{incident.title}</p><p className="mt-1 text-xs text-slate-500">{formatDate(incident.createdAt)}{incident.isPrimary ? " · primary service" : ""}</p></div>
                    <div className="flex gap-2"><PriorityBadge priority={incident.priority} /><StatusBadge status={incident.status} /></div>
                  </Link></li>
                ))}
              </ul>
            )}
        ...
        {session.permissions.includes("services.manage") ? (
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900 p-5">
            ...
            <p className="mt-1 text-sm text-slate-400">Create a stable or ephemeral deployment context for this service.</p>
            <div className="mt-6"><CreateEnvironmentForm serviceId={service.id} /></div>
          </section>
