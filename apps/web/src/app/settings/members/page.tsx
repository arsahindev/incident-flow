    ...
    <main className="mx-auto max-w-6xl px-6 py-12">
      <div>
        ...
        <p className="mt-2 text-slate-400">
          Manage organization roles, access state, and team membership.
        </p>
      </div>
      <div className="mt-10 grid items-start gap-8 lg:grid-cols-[1fr_22rem]">
            ...
            {members.map((member) => (
              <li key={member.userId} className="space-y-5 px-6 py-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {teams.map((team) => {
                    const assigned = member.teams.some(
                      (candidate) => candidate.id === team.id,
                    );
                    const action = updateTeamMembershipAction.bind(
                      null,
                      team.id,
                      member.userId,
                      !assigned,
                    ...
                    return (
                      <form action={action} key={team.id}>
                        <button
                          className={`rounded-full border px-3 py-1 text-xs ${assigned ? "border-cyan-500/50 bg-cyan-500/10 text-cyan-200" : "border-slate-700 text-slate-400"}`}
                        >
                          {assigned
                            ? `Remove ${team.name}`
                            : `Add ${team.name}`}
        ...
        <aside className="rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Invitation secrets are stored only as hashes and expire after 48
            hours.
          </p>
          <div className="mt-6">
            <InvitationForm />
