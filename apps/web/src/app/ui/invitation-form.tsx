  ...
  return (
    <form action={action} className="space-y-4">
      <label className="block text-sm text-slate-300">Email
        <input name="email" type="email" required autoComplete="off" className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5" />
      </label>
      <label className="block text-sm text-slate-300">Role
        <select name="role" defaultValue="responder" className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5">
      ...
      {state.invitationUrl ? (
        <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-200">
          ...
          <Link href={state.invitationUrl} className="mt-2 block break-all font-mono text-xs underline">
            {state.invitationUrl}
          </Link>
        </div>
      ) : null}
