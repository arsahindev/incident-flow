...
if (process.env.SMOKE_PASSWORD) {
  ...
  if (process.env.SMOKE_EXPECT_PUBLIC_DEMO === 'true') {
    assert(html.includes(process.env.SMOKE_EMAIL), 'Public demo email is displayed');
    assert(html.includes(process.env.SMOKE_PASSWORD), 'Public demo password is displayed');
  }
  ...
  const session = login.headers.getSetCookie().find(x=>x.startsWith('incidentflow_session='));
  assert(session?.includes('HttpOnly') && session.includes('Secure'), 'Secure HttpOnly cookie');
  const cookie = session.split(';')[0];
  const dashboard = await fetch(origin, {headers:{Cookie:cookie}, redirect:'manual'});
  assert.equal(dashboard.status, 200, 'Authenticated dashboard');
  console.log('Login and authenticated dashboard: PASS (Secure, HttpOnly cookie)');
  for (const transport of ['polling', 'fallback']) {
    await new Promise((resolve,reject)=> {
      const socket = io(origin, {transports:transport === 'fallback' ? ['websocket', 'polling'] : [transport],tryAllTransports:true,extraHeaders:{Origin:origin,Cookie:cookie},reconnection:false,timeout:15000});
      socket.once('connect', ()=>{console.log(`Authenticated Socket.IO ${transport}: PASS`);socket.disconnect();resolve();});
      socket.once('connect_error', ()=>{socket.disconnect();reject(new Error(`Authenticated Socket.IO ${transport}: FAILED`));});
    });
  }
}
